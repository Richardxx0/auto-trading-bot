...
**当前版本特性：**
- Glassmorphism 毛玻璃设计，一键深色/浅色主题切换
...
- SocketIO 实时日志流
- 每 5 秒自动刷新全盘数据
- 一键全额清仓按钮及逐行平仓操作（需后端支持）

> 此版本替换了原有 Bootstrap 风格模板。旧看板为 Bootstrap 5.3 + 内联 SocketIO + 暗色主题的简单实现，仅包含信号表、交易记录表和日志控制台。新版本在保留全部功能基础上，升级了视觉设计并新增 ECharts 图表分析、主题切换、核心信号追踪等功能。
