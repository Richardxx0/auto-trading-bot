...
| ATR 倍数 | 1.5 | 动态模式：SL = 最高价 - ATR×1.5（设0回退固定百分比） |
| 回撤距离(回退) | 最高价 - 2% | 从最高价回落 2% 即止损 |

```
例：开仓价 100 → 涨到 110 → SL 上移到 107.8
    → 涨到 115 → SL 上移到 112.7
    → 跌到 112.5 → 触发止损, 锁定 12.5% 利润
ATR 自适应：高波动币种（ATR大）止损宽，低波动币种（ATR小）止损紧
```
...
| 成交量过滤 | 当前成交量 < Volume MA20 则跳过，减少假突破 |
| 持久化去重 | 已处理的币种写入 `dedup.json`，重启不重复开仓 |
| 内存去重 | 同一次运行中不重复开同一个币 |
| 持仓检查 | 开仓前查询交易所，已有持仓则跳过 |
| 分批止盈 | 50% + 30% + 20% 三批止盈 |
| 移动止损(ATR) | 浮盈 3% 后启动，按 ATR 动态跟踪 |
| 连续亏损熔断 | 连续亏损 3 次暂停交易 4 小时 |
| 每日亏损上限 | 当日亏损超 3% 停止当日交易 |
