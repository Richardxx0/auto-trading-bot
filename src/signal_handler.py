...
class SignalHandler:
    ...
    async def _处理信号(self, sender: str, text: str) -> None:
        ...
        if self._exchange.has_open_position(contract_symbol):
            # 奔跑模式仓位不阻止新开仓
            _runner_trades = [t for t in ts.load_all() if t.get("symbol") == contract_symbol and t.get("status") == "OPEN" and t.get("runner")]
            if _runner_trades:
        ...
        if effective_count >= self._cfg.max_open_positions:
        ...
        # 测试网成交量不稳定，自动跳过确认；实盘正常检查
        if self._cfg.binance_testnet:
            logger.info("【成交量过滤】测试网模式，自动跳过成交量确认")
        elif self._cfg.volume_confirm_enabled and not analysis.get("error"):
    ...
    async def _执行市价开仓(
        ...
        # ATRèªéåº + éç½®é£æ§ä¸é

        if analysis and analysis.get("sl_price") and analysis.get("tp_levels"):
            risk_sl = risk_result["sl"]
            risk_tp = risk_result["tp"]
            sl_price = max(analysis["sl_price"], risk_sl)
            tp_levels = []
            for lv in analysis["tp_levels"]:
                capped = min(lv["price"], risk_tp)
                tp_levels.append({"price": capped, "qty_pct": lv["qty_pct"], "label": lv["label"]})
            logger.info("  é£æ§æ¶æSL/TP: SL=%.8f  TP=%s",
                        sl_price,
                        ", ".join(f"{lv[chr(108)+chr(97)+chr(98)+chr(101)+chr(108)]}={lv[chr(112)+chr(114)+chr(105)+chr(99)+chr(101)]}"
                                  for lv in tp_levels))
        ...
        if "error" in result and result["error"]:
            ...
            tp1 = tp_levels[0].get("price", 0) if isinstance(tp_levels, list) and len(tp_levels) > 0 else 0

            tp2 = tp_levels[1].get("price", 0) if isinstance(tp_levels, list) and len(tp_levels) > 1 else 0

            tp3 = tp_levels[2].get("price", 0) if isinstance(tp_levels, list) and len(tp_levels) > 2 else 0
    ...
    async def _执行限价开仓(
        ...
        asyncio.create_task(
            ...
            self._监控限价单(symbol, order_id, qty, sl_price, tp_levels)
    ...
    async def _监控限价单(
        ...
        while elapsed < timeout:
            ...
            if status in ("closed", "filled"):
                ...
                # 直接调 _挂止盈止损，不走 set_stop_loss_take_profit

                results = {"stop_loss": None, "take_profits": []}

                self._exchange._挂止盈止损(symbol, filled_qty, sl_price, tp_levels, results)

                if results.get("stop_loss"):
